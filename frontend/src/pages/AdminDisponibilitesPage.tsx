...
const AdminDisponibilitesPage = () => {
  ...
  const { data: disponibilites, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Filtrer pour exclure les disponibilités des admins
      return allDisponibilites.filter((disp) => disp.user.userRole !== 'admin');
    },
  });
  ...
  const disponibilitesByMedecin = filteredDisponibilites
    ?.filter((disp) => disp.user.userRole !== 'admin') // Double vérification pour exclure les admins
    .reduce((acc, disp) => {
                    ...
                    {data.disponibilites.map((disp) => (
                      <tr key={disp.id} className="hover:bg-gray-50">
                        ...
                        <td className="whitespace-nowrap px-6 py-4">
                          {disp.estException
                            ? disp.dateSpecifique
                              ? format(new Date(disp.dateSpecifique), 'dd/MM/yyyy')
                              : '-'
...
export default AdminDisponibilitesPage;
